...
st.title("⛳ AI 정밀 골프 스윙 분석 시스템 (기하학적 절대 검색 엔진)")
st.write("주먹구구식 중간값(Midpoint) 추정을 전면 폐기했습니다. 모든 프레임의 관절과 샤프트 각도를 계산하여 '지면과 완벽히 수평(0도/90도)'이 되는 찰나를 수학적으로 도출합니다.")
...
if uploaded_file is not None:
    ...
    if st.button("전체 프레임 기하학적 정밀 분석 시작", type="primary"):
        with st.spinner("모든 프레임의 팔 수평, 수직, 샤프트 각도를 계산하여 매칭 중입니다... (약 1분 소요)"):
            ...
            for frame in frames_bgr:
                ...
                if res.pose_landmarks:
                    ...
                    # [핵심] 샤프트 픽셀 전수 추적 (OpenCV)
                    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                    blurred = cv2.GaussianBlur(gray, (5, 5), 0)
                    edges = cv2.Canny(blurred, 30, 150)
                    mask = np.zeros_like(edges)
                    cv2.circle(mask, (int(hx), int(hy)), 200, 255, -1) # 손목 주변만 집중 스캔
                    ...
                    for arr in [wrist_ys, wrist_xs, lw_ys, ls_ys, rw_ys, rs_ys, rw_xs, re_xs, shaft_angles]:
                        arr.append(np.nan)
            
            st.session_state.lm_data = landmarks_data
            
            # --- 3. 데이터 보간 (모션 블러로 사라진 샤프트 각도 복원) ---
            def smooth(arr):
                return pd.Series(arr).interpolate(limit_direction='both').rolling(3, center=True, min_periods=1).mean().values
            ...
            # 360도 래핑 방지 원형 보간 (Trigonometric Interpolation)
            sin_a = pd.Series([math.sin(math.radians(a)) if not np.isnan(a) else np.nan for a in shaft_angles]).interpolate(limit_direction='both').rolling(3, center=True, min_periods=1).mean()
            cos_a = pd.Series([math.cos(math.radians(a)) if not np.isnan(a) else np.nan for a in shaft_angles]).interpolate(limit_direction='both').rolling(3, center=True, min_periods=1).mean()
            shaft_smooth = (np.degrees(np.arctan2(sin_a, cos_a)) % 360).values

            # --- 4. 기하학적 절대 검색 (No Midpoints!) ---
            ...
            def find_angle_frame(start, end, target, angles):
                if start >= end: return start
                sub = angles[start:end]
                diffs = [min(abs(a - target), 360 - abs(a - target)) for a in sub]
                return start + np.argmin(diffs)

            # 앵커
            f_p5 = int(np.nanargmin(w_y)) # Top
            f_p1 = int(np.nanargmax(w_y[:f_p5])) if f_p5 > 0 else 0 # Address
            f_p8 = f_p5 + int(np.nanargmax(w_y[f_p5:min(total_frames, f_p5 + int(fps * 1.5))])) # Impact
            f_p13 = min(total_frames - 1, f_p8 + int(fps * 1.0)) # Finish

            # 완벽한 팔 지면 수평/수직 검색
            f_p4 = find_min_diff_frame(f_p1, f_p5, lw_y, ls_y) # [P4] 왼팔 수평 (어깨Y == 손목Y)
            f_p11 = find_min_diff_frame(f_p8, f_p13, rw_y, rs_y) # [P11] 오른팔 수평 (어깨Y == 손목Y)
            f_p12 = find_min_diff_frame(f_p11, f_p13, rw_x, re_x) # [P12] 오른팔 수직 (팔꿈치X == 손목X)

            # 완벽한 샤프트 각도 검색
            f_p3 = find_angle_frame(f_p1, f_p5, 90, shaft_smooth) # [P3] 샤프트 평행
            f_p2 = find_angle_frame(f_p1, f_p3, 45, shaft_smooth) # [P2] 샤프트 45도
            
            f_p7 = find_angle_frame(f_p5, f_p8, 90, shaft_smooth) # [P7] 샤프트 평행 (다운스윙)
            f_p6 = find_angle_frame(f_p5, f_p7, 135, shaft_smooth) # [P6] 샤프트 135도
            
            f_p10 = find_angle_frame(f_p8, f_p13, 270, shaft_smooth) # [P10] 샤프트 270도 (평행)
            f_p9 = find_angle_frame(f_p8, f_p10, 315, shaft_smooth) # [P9] 샤프트 315도
...
if st.session_state.get('analyzed'):
    st.success("✅ 기하학적 절대 검색 완료! 팔이 수평이 되고 샤프트가 45도, 90도가 되는 수학적 찰나를 도출했습니다.")
    ...
    csv_placeholder.download_button("💾 현재 설정된 프레임 데이터 CSV 다운로드", data=csv, file_name='geometric_scan_swing.csv', mime='text/csv', type='primary')
